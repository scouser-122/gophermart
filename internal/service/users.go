...
func NewUsersService(
	userGenStorage repository.UsersStorage,
	ordersStorage repository.OrdersStorage,
) *UsersService {
	service := UsersService{}
	service.usersStorage = userGenStorage
...
func (service *UsersService) Register(ctx context.Context, user *models.User) (*models.User, error) {
	...
	newUser, err := service.usersStorage.Create(ctx, user.Login, user.Password)
	if err != nil {
		return nil, err
	}
	return newUser, nil
...
func (service *UsersService) GetUserBalance(ctx context.Context, login string) (*models.UserBalanceResponse, error) {
	...
	var err error
	storedUser, err := service.usersStorage.Get(ctx, login)
	if err != nil {
		return nil, err
	}
	result.Current = storedUser.Balance
	result.Withdrawn, err = service.ordersStorage.GetWithdrawnForUser(ctx, login)
	if err != nil {
		return nil, err
	}
	return &result, nil
